...
### CI/CD scope: application code only, infra stays manual

Two workflows, one per concern:

- `deploy-03-serverless.yml` zips and pushes Lambda code on every push to
  `backend/lambda/**`.
- `deploy-03-frontend.yml` builds the React app and syncs it to the existing
  S3 bucket + invalidates the existing CloudFront distribution on every push
  to `frontend/**`.

Neither runs `terraform apply`. This mirrors Phase 2's approach and is a
direct consequence of the IAM decisions above: the CI user's two scoped
policies only grant `lambda:UpdateFunctionCode` and
`s3:PutObject`/`DeleteObject`/`GetObject`/`ListBucket` +
`cloudfront:CreateInvalidation` on Phase 3's *specific* resources — never the
broader Cognito/API Gateway/DynamoDB/IAM-role/S3-bucket-creation/CloudFront-
distribution-creation permissions a full `terraform apply` would need. Giving
CI those permissions was considered and rejected — it would mean a
compromised GitHub Actions run (or a bad PR from a fork) could restructure
the AWS account, not just overwrite application code. Infra changes go
through a human running `terraform plan`/`apply` locally. Splitting Lambda
and frontend into separate workflows (rather than one combined one) also
means a frontend-only change doesn't redeploy Lambda and vice versa, and a
failure in one is immediately obvious from which workflow failed.
...
- The `reflect` tab's "day N" badge is computed at render time from the
  ...
  [`Frontend-Design.md`](./Frontend-Design.md) for why, and the edge case it
  fixes.
...
付与したPhase 3用ポリシー（`iam.tf`）は2本あり、それぞれPhase 3の4つの
Lambda関数ARNに対する`lambda:UpdateFunctionCode`、およびPhase 3専用の
S3バケット/CloudFrontディストリビューションに対する書き込み・Invalidation権限
のみに厳密に限定されており、Phase 1/2のリソースやPhase 3の他のリソース種別
（インフラそのものの作成等）には一切触れられない。

### CI/CDのスコープ: アプリケーションコードのみ、インフラは手動のまま

関心事ごとに2つのワークフローに分けている:

- `deploy-03-serverless.yml`は`backend/lambda/**`へのpush時にLambdaコードを
  zip化してデプロイする
- `deploy-03-frontend.yml`は`frontend/**`へのpush時にReactアプリをビルドし、
  既存のS3バケットへ同期・既存のCloudFrontディストリビューションを無効化する

どちらも`terraform apply`は実行**しない**。これはPhase 2の方針を踏襲しており、
上記のIAM判断の直接の帰結でもある：CI用ユーザーが持つ2つの限定ポリシーは
`lambda:UpdateFunctionCode`と、Phase 3の**特定の**S3バケット/CloudFront
ディストリビューションに対する`s3:PutObject`/`DeleteObject`/`GetObject`/
`ListBucket`＋`cloudfront:CreateInvalidation`のみで、完全な`terraform apply`に
必要なCognito/API Gateway/DynamoDB/IAMロール作成/S3バケット作成/CloudFront
ディストリビューション作成の権限は一切持たない。CIにこれらの権限を持たせる案も
検討したが却下した — GitHub Actionsの実行が侵害された場合（あるいはフォークからの
悪意あるPR）、アプリケーションコードの上書きだけでなくAWSアカウントの構成
そのものを変更されてしまうリスクがあるため。インフラの変更は人間がローカルで
`terraform plan`/`apply`を実行する形を維持する。Lambdaとフロントエンドを
1つに統合せず別ワークフローにしているのは、フロントエンドだけの変更で
Lambdaが再デプロイされない（逆も同様）ようにするためと、どちらが失敗したかが
ワークフロー名だけで即座に分かるようにするため。
...
- 気づきタブの「N日目」バッジは、作成時に固定値として保存するのではなく、
  ...
  修正した具体的な不具合については[`Frontend-Design.md`](./Frontend-Design.md)を参照。
