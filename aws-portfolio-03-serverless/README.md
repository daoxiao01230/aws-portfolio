...
```
React (aws-portfolio-01-static-site/react)
  │  login / signup
  ▼
...
- [ ] Reactフロントにログイン/サインアップ画面追加（amazon-cognito-identity-js or Amplify Auth）
- [ ] React側でCRUD画面（一覧・追加・編集・削除）実装、JWTをAuthorizationヘッダーに付与
- [ ] エンドツーエンド動作確認（サインアップ→ログイン→日記のCRUD）
